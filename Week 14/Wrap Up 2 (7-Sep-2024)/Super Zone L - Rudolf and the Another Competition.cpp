...
//        print_all(pref);
        auto it = upper_bound(all(pref), h);
        it--;
        ll idx = it - pref.begin();
        pos[i].first = -idx;
        pos[i].second = 0;
        REPF(z, 0, (-pos[i].first) - 1) {
            pos[i].second += idx * a[z];
            idx--;
        }
        if (i == 0)
            rud.first = pos[i].first, rud.second = pos[i].second;
    }

//    print_pairs(pos);
    sort(all(pos));

    auto it = lower_bound(all(pos), rud);
    ll ans = (it - pos.begin()) + 1;
    cout << ans << nl;
}
