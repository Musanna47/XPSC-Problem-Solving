...
ll dfs(ll curr, vpll &nodes, string &s) {
    if (nodes[curr].first == 0 && nodes[curr].second == 0)
        return 0;
    ll lans = nodes[curr].first ? dfs(nodes[curr].first, nodes, s) : INF;
    ll rans = nodes[curr].second ? dfs(nodes[curr].second, nodes, s) : INF;
    ll mn = min(lans, rans);
    if (lans == mn && s[curr - 1] == 'L')
        return mn;
    if (rans == mn && s[curr - 1] == 'R')
        return mn;
    return 1 + mn;
}
