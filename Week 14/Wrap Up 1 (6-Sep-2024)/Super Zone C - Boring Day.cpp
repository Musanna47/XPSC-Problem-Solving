...
struct SEG {
    ...
    void init(int _n) {
        n = _n;
        fill(tree.begin(), tree.begin() + 4 * n + 5, 0);
        fill(lazy.begin(), lazy.begin() + 4 * n + 5, 0);
    }
    ...
    void build(vi &a, int node, int start, int fin) {
        if (start == fin) {
            tree[node] = a[start - 1];
        } else {
            int mid = (start + fin) / 2;
            build(a, 2 * node, start, mid);
            build(a, 2 * node + 1, mid + 1, fin);
            tree[node] = tree[2 * node] + tree[2 * node + 1];
        }
    }

    void updateRange(int node, int start, int fin, int l, int r, ll val) {
        if (lazy[node]) {
        ...
        updateRange(node * 2 + 1, mid + 1, fin, l, r, val);
        tree[node] = tree[node * 2] + tree[node * 2 + 1];
    }

    ll queryRange(int node, int start, int fin, int l, int r) {
        if (start > fin or start > r or fin < l)
            return 0;
        if (lazy[node]) {
            tree[node] += (fin - start + 1) * lazy[node];
        ...
        auto p2 = queryRange(node * 2 + 1, mid + 1, fin, l, r);
        return (p1 + p2);
    }
};
...
void solve() {
    ...
    seg.init(n);
    REPF(i, 0, n - 1) {
        seg.updateRange(1, 1, n, i + 1, n, a[i]);
    }
    ...
    REPF(i, 0, n - 1) {
        ...
        while (l <= r) {
            ll mid = (l + r) / 2;
            ll val = a[i] + seg.queryRange(1, 1, n, mid, mid);
            if (val < LT) {
                l = mid + 1;
            } else if (val > RT) {
                r = mid - 1;
