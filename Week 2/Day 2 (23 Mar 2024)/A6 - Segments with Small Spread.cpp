...
void solve() {
    ...
    while (l < n) {
        while (r < n &&
               (window.empty() || (a[r] <= *window.rbegin() && a[r] >= *window.begin()) ||
                (a[r] > *window.rbegin() && a[r] - *window.begin() <= k) ||
                (a[r] < *window.begin() && *window.rbegin() - a[r] <= k))) {
            window.insert(a[r]);
            r++;
        }
        if (!window.empty() && *window.rbegin() - *window.begin() <= k)
            ans += sza(window);
        window.extract(a[l]);
        l++;
    }
