...
void solve() {
    ...
    map<char, vi > ind;
    REPB(i, n - 1, 0) ind[s[i]].pb(i);

    ordered_set<pair<int, char>> oset;
    REPF(i, 0, n - 1) oset.insert({i, s[i]});

    ll ans = 0;
    int neg = -1;
    REPF(i, 0, n - 1) {
        if (oset.find_by_order(i)->second == t[i]) {
            ind[t[i]].pop_back();
            continue;
        }
        int nxt = ind[t[i]].back();
        auto it = oset.find({nxt, t[i]});
        ll r = oset.order_of_key(*it);
        ans += r - i;
        oset.insert({neg, it->second});
        neg--;
        oset.erase(it);
        ind[t[i]].pop_back();
    }
