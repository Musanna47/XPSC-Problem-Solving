...
void solve() {
    ...
    if (oddIdx != -1) {
        result.push_back(oddIdx + 'A');
        chCount[oddIdx]--;
    }
    string palHalf1, palHalf2;
    for (int i = 0; i < 26; i++) {
        for (int j = chCount[i] / 2; j > 0; j--) {
            palHalf1.push_back(i + 'A');
            palHalf2.push_back(i + 'A');
        }
    }
    reverse(all(palHalf2));
    result = palHalf1 + result + palHalf2;
    cout << result;
}
