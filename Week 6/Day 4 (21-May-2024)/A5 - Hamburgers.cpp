...
void solve() {
    ...
    cin >> money;
    ll l = 0, r = 0, ans = 0;
    r = max(r, currB + money / priceB);
    r = max(r, currS + money / priceS);
    r = max(r, currC + money / priceC);
    while (l <= r) {
        auto mid = (l + r) / 2;
        auto ok = [&]() -> bool {
            ll reqB = max(0LL, b * mid - currB);
            ll reqS = max(0LL, s * mid - currS);
            ll reqC = max(0LL, c * mid - currC);
            ll rem = money;
            rem -= reqB * priceB;
            rem -= reqS * priceS;
            rem -= reqC * priceC;
